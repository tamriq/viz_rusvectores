...
def get_most_similar(model, word, topn = 10):
    ...
    pairs = [(neighbors[ab], neighbors[ba]) for ab in range(len(neighbors)) for ba in range(ab+1,len(neighbors))]
    for pair in pairs: arr.append({"source": pair[0], "target": pair[1], "value":model.similarity(*pair)})
    return [arr, neighbors]
    
def make_html(word, data, interlinks = [], edge = 1):
    return html.replace("wordplaceholder", word).replace("dataplaceholder", str(data)).replace("linksplaceholder", "pages = "+str(interlinks)).replace("linkstrokewidth;", "linkstrokewidth = "+str(edge)+";")
...
def visualize_file(filename, model, word, topn=10, edge=1):
    if not filename:
        raise ValueError('filename is empty')
    html = visualize_str(model, word, depth, edge)
...
def visualize_dir(dir, model, word, depth=0, topn=10, edge=1):
    ...
    for k in pages:
        ...
        print(html, file=open(os.path.join(dir, k+".html"), "w", encoding='utf-8'), end='')
    pass
