...
parser.add_argument(
    "-m",
    "--model",
    help="path to vector model file. If omitted, first model with extension "
...
parser.add_argument(
    "-o",
    "--output",
    help="path to output directory where to store files of visualization."
         " If omitted, in current directory new one will be made, with a name"
...
if not args.output:
    ...
    new_dir = os.path.join(directory, dt)
    pathlib.Path(new_dir).mkdir(parents=True, exist_ok=True)
    args.output = new_dir

if not args.model:
    first = glob.glob(os.path.join(directory, "*.[vecbin]*.gz"))[0]
    args.model = first
