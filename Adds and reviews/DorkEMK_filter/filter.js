...
function formResultsUsingFilterList(resultArray){ 
  console.log(resultArray)
  $("ol").each(function(){ // TODO - проверить, что <ol> вообще подходящий
...
$(document).ready(function(){
  const RESULTS = JSON.parse($("ol").data("frequencies").replace(/'/g, '"'));
  const FREQUENCIES = ['high', 'mid', 'low'];
  const MODELS = Object.keys(RESULTS);
  const FILTER = 'freq' // TODO массив фильтров
  ...
  formResultsUsingFilterList(output);
